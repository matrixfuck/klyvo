...
Формат Cursor: команда в поле `command` верхнего уровня, рабочая директория в
`cwd` (её и берём за корень проекта — отдельного поля с корнем воркспейса в
этом событии нет). Ответ — {"permission": "allow|deny|ask", "user_message",
"agent_message"}. Модель `ask` та же, что у Claude Code, — «мягкое» подтверждение.
...
def main():
    ...
    print(json.dumps({
        "permission": decision,
        "user_message": reason,   # актуальные имена полей Cursor (snake_case)
        "agent_message": reason,
        "userMessage": reason,    # дубль на случай иной версии формата
        "agentMessage": reason,
    }, ensure_ascii=False))
    sys.stdout.flush()
