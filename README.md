...
### Общий вход для локального дашборда (`--central-auth`)

Вместо `--set-password` дашборд может проверять логин/пароль на центральном
сервере (например, [klyvo.tech/hub](https://klyvo.tech/hub)) — один раз при
входе, дальше обычная локальная сессия (30 дней), без повторных запросов:

```
python3 klyvo_web.py --central-auth https://klyvo.tech/hub --project ~/app
```

Данные (`.klyvo/`) остаются полностью локальными — на сервер уходит только
логин/пароль в момент входа, ничего больше. Пароль нигде локально не хранится:
только случайный секрет для подписи собственной сессионной куки
(`~/.klyvo-web/local-secret.json`). Сервер недоступен — просто нельзя войти,
на работу самого guard'а (перехват команд) это никак не влияет.

### Онлайн-хостинг для тестеров (`--multiuser`)

Отдельный режим — общий дашборд на много аккаунтов, у каждого свои данные,
никто не видит чужие:

```
python3 klyvo_web.py --multiuser --port 8766
```

Логин/пароль без почты (восстановить пароль пока нельзя — храните сами).
Данные не приходят автоматически: пользователь сам собирает файл
(`python3 klyvo_journal.py --export`) и загружает его через веб-форму
(`/upload`) — то есть каждый явно решает, что и когда отправить. rate-limit на
регистрацию/вход/проверку пароля — простой, в памяти процесса, для масштаба
беты этого достаточно. На klyvo.tech этот режим живёт на `/hub/` — отдельный
процесс и порт от личного `/dashboard/`, они не пересекаются.
...
python3 tests/test_web.py      # сбор данных, авторизация single/multiuser/central-auth, изоляция данных
