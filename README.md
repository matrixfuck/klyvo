...
Перехватывает разрушительные команды с данными до того, как их выполнит
агент (Claude Code) — от базы до тома у хостинг-провайдера. Критичное (удаление таблиц, массовое удаление, сброс
...
ядре `klyvo/rules.py` (53 правила и 2 эвристики), чтобы её можно было
...
Отдельно — узкий набор правил на катастрофичное удаление файлов: `rm -rf /`,
снос домашнего каталога, затирание диска через `mkfs` или `dd`. **Klyvo не пытается
контролировать `rm` вообще** и сознательно не трогает обычную уборку: `rm -rf ~/projects/old`,
`rm -rf ./build`, чистка кешей — всё это проходит свободно. Блокируется только то,
где цель это корень и восстанавливать нечего.
