...
**Он обходится тривиально.** Склейка `"DR" "OP TABLE"`, сборка команды через
переменную, base64, `find . -name '*.db' -delete` вместо `rm` — ни один из этих
вариантов не совпадёт с паттерном. Это не список недоработок, а предел подхода:
чтобы их закрыть, нужно понимать смысл команды, а не её текст.
