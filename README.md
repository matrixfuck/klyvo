...
агент — от базы до тома у хостинг-провайдера. Критичное (удаление таблиц,
массовое удаление, сброс миграций) блокирует, на остальное просит подтверждение.
Работает даже когда агент запущен в режиме автоодобрения. Каждую перехваченную
команду пишет в журнал.

Ставится в Claude Code, Cursor, Codex CLI, Kimi Code, opencode и форки Claude
Code. Живьём проверен пока только Claude Code — по остальным честный статус в
[таблице агентов](#поддерживаемые-агенты).
...
ядре `klyvo/rules.py` (55 правил и 2 эвристики), чтобы её можно было
...
## Проверить, что защита действительно работает

Хук намеренно fail-open: если он сам сломается, агент продолжит работать. Это
правильно для повседневной работы, но означает, что «ничего не происходит»
выглядит одинаково и когда всё хорошо, и когда защиты давно нет — например,
после переноса каталога или обновления Python.

Поэтому есть самопроверка:

```bash
python3 ~/klyvo/klyvo_rules.py doctor
```

Она проверяет весь путь, а не правила в отрыве: загружается ли ядро, отвечает ли
**реальный хук** отказом на заведомо разрушительную команду, прописан ли он в
конфигах ваших агентов и не было ли недавних сбоев. Код возврата ненулевой, если
что-то не так, — команду можно повесить в CI или в свой скрипт запуска.

Сбои guard записывает сам в `~/.klyvo/health.jsonl`, и сводка сессии показывает
их первой строкой: «Klyvo давал сбой N раз — в эти моменты команды проходили без
проверки». Так «опасных операций не перехвачено» нельзя спутать с «никто не
смотрел».

## Чего Klyvo не умеет

Движок сигнатурный: он читает текст команды и не понимает её смысла. Отсюда два
следствия, и второе важнее первого.

**Он реагирует на упоминание, а не только на исполнение.** Безобидная команда,
внутри которой есть триггерное слово — `echo` с примером SQL, коммит-сообщение,
патч документации, — будет перехвачена. Как с этим жить, написано ниже.

**Он обходится — но уже не тривиально.** Перед проверкой команда
разворачивается: снимаются кавычки и экранирование, подставляются переменные,
присвоенные в этой же команде, расшифровывается base64. Поэтому склейка
`"DR""OP TABLE users"`, сборка вида `CMD=...; bash -c "$CMD"` и
`echo <base64> | base64 -d | psql` перехватываются наравне с обычной записью, а
удаление через `find -delete` покрыто отдельными правилами.

Но разворачивание — не интерпретатор shell. Подстановка команд `$(...)`, сборка
строки внутри Python или Node, скачанный и запущенный скрипт, свой бинарник
вместо `psql` — проходят по-прежнему. Закрыть этот класс целиком нельзя: для
этого нужно понимать, что команда сделает, а не как она написана.

Отсюда честная область применения: **Klyvo защищает от спешки, а не от умысла.**
Против агента, который торопится и не думает о последствиях, он работает. Против
агента, который целенаправленно подбирает способ выполнить отклонённую команду, —
нет. И он не отменяет ни бэкапы, ни разграничение прав в базе: это слой поверх
них, а не замена.

Не покрывается и удаление данных через ваш собственный API: если агент дёрнет
`DELETE /api/users` у боевого бэкенда, Klyvo увидит обычный HTTP-запрос.
